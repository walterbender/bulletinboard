...
def save_svg(activity,  nick):
    ''' Output a SVG document from the title, pictures, and descriptions '''

    if len(activity.dsobjects) == 0:
        return None

    tmp_file = os.path.join(activity.datapath, 'output.svg') 
    svg_surface = cairo.SVGSurface(tmp_file, 504, 648)

    fd = pango.FontDescription('Sans')
    cr = cairo.Context(svg_surface)
